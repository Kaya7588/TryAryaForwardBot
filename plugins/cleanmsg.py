...
async def _do_delete(client, chat_id: int, wanted: str,
                     status_msg, is_bot: bool) -> int:
    """
    ...
    async def flush():
        ...
        if batch:
            total += await _safe_delete(client, chat_id, batch)
            batch = []
            try:
                await status_msg.edit_text(
                    f"<b>🗑 Cleaning… <code>{total}</code> deleted so far.</b>"
                )
            except Exception:
                pass
...
async def _cleanmsg_flow(bot, user_id: int):
    ...
    type_reply = await bot.ask(
        user_id,
        "<b>🗑 Step 3/3</b> — What type of messages to delete?",
        reply_markup=ReplyKeyboardMarkup([
            ...
            [KeyboardButton("📦 All Media"),      KeyboardButton("/cancel")],
    ...
    await type_reply.reply("<b>Confirm above ⬆️</b>", reply_markup=ReplyKeyboardRemove())
...
async def cleanmsg_confirm(bot, query):
    ...
    for chat_id in selected_chats:
        try:
            ...
            count = await _do_delete(client, chat_id, wanted, status_msg, is_bot)
            grand_total += count
        except Exception as e:
